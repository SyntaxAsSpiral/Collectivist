...
Usage: python src/__main__.py <command> [options]
...
def get_collection_path() -> Path:
    """Get collection path - seed folder is self-contained"""
    # From collectivist-portable/src/__main__.py -> collectivist-portable/src/ -> collectivist-portable/
...
def cmd_analyze(args):
    ...
    try:
        ...
        if not test_llm_connection(llm_client):
            ...
            print("  Configure LLM provider in llm-config.yaml (in .collection directory)")
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="python src/__main__.py",
        description="Collectivist - AI-powered collection curator",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python src/__main__.py analyze              # Detect collection type
  python src/__main__.py scan                 # Scan for items and metadata
  python src/__main__.py describe             # Generate descriptions
  python src/__main__.py render               # Generate README.md
  python src/__main__.py update               # Run full pipeline
  
  python src/__main__.py analyze --force-type repositories
  python src/__main__.py describe --max-workers 10
  python src/__main__.py update --skip-analyze --skip-scan
        """
    )
